...
class LateFusionAgent(autonomous_agent.AutonomousAgent):
	...
	def get_image_encoder_model(self):
		return self.net.image_encoder

	def get_lidar_encoder_model(self):
		return self.net.lidar_encoder
	...
	def preprocess_lidar(self, tick_data, lidar):
		#print('Lidar original: ', tick_data['lidar'])
		#print('LIDAR original max: ', torch.max(torch.from_numpy(tick_data['lidar'])))

		#print('In lidar preprocess test#1: ', self.step, self.config.seq_len, len(self.input_buffer['lidar']), len(self.input_buffer['gps']), len(self.input_buffer['thetas']))
		...
		#print('In lidar preprocess test#2: ', self.step, self.config.seq_len, len(self.input_buffer['lidar']), len(self.input_buffer['gps']), len(self.input_buffer['thetas']))
		...
		for i, lidar_point_cloud in enumerate(self.input_buffer['lidar']):
			...
			#print('ego_theta: {}, ego_x: {}, ego_y: {}, curr_theta: {}, curr_x: {}, curr_y: {}'.format(
			#		ego_theta, ego_x, ego_y, curr_theta, curr_x, curr_y))
			lidar_point_cloud[:,1] *= -1 # inverts x, y
			#print('Lidar point cloud: ', lidar_point_cloud)
			lidar_transformed = transform_2d_points(lidar_point_cloud,
						np.pi/2-curr_theta, -curr_x, -curr_y, np.pi/2-ego_theta, -ego_x, -ego_y)
			#print('Lidar transformed: ', lidar_transformed)
			lidar_hist = lidar_to_histogram_features(lidar_transformed, crop=self.config.input_resolution)
			#print('Lidar hist: ', lidar_hist.shape)
			'''
			for ch in range(lidar_hist.shape[0]):
				for r in range(lidar_hist.shape[1]):
					for c in range(lidar_hist.shape[2]):
						if lidar_hist[ch][r][c]>0.0:
							print(ch, r, c, lidar_hist[ch][r][c])
			'''
			lidar_transformed = torch.from_numpy(lidar_hist).unsqueeze(0)
			#print('Lidar transformed after hist: ', lidar_transformed)
			#print('Lidar transformed: ', lidar_transformed[lidar_transformed>0.0])
			#print('Lidar transformed max: ', torch.max(lidar_transformed))
		...
		#print('In lidar preprocess test#3: ', self.step, self.config.seq_len, len(self.input_buffer['lidar']), len(self.input_buffer['gps']), len(self.input_buffer['thetas']))
	...
	'''
	@torch.no_grad()
	def get_image_features(self, rgb):
		rgb = Image.fromarray(rgb).convert('RGB')
		rgb = torch.from_numpy(scale_and_crop_image(rgb, crop=self.config.input_resolution)).unsqueeze(0)
		rgb = rgb.to('cuda', dtype=torch.float32)
		rgb /= 255.0
		print('In: ', rgb.shape)
		print('In data: ', rgb[0])
		out = self.net.image_encoder(rgb)
		out = torch.argmax(out)
		out = torch.LongTensor([out])
		#out = torch.LongTensor([0])

		print('Out: ', out)
		#print('Out argmax: ', torch.max(out, 1))
		return rgb, out
	'''

	@torch.no_grad()
	#def run_step(self, input_data, timestamp):
	def run_step(self, tick_data, timestamp):
		...
		for i, lidar_point_cloud in enumerate(self.input_buffer['lidar']):
			...
			lidar_processed.append(lidar_transformed.to('cuda', dtype=torch.float32))
		'''
		lidar_processed = [tick_data['lidar_preprocessed']]
